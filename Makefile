...
PROJECT="reForis Remote Wi-Fi Settings" 
...
.PHONY: lint-web
lint-web: venv
	$(VENV_BIN)/$(PYTHON) -m ruff check reforis_remote_wifi_settings
...
create-messages: venv
	$(VENV_BIN)/pybabel extract -F babel.cfg -o ./reforis_remote_wifi_settings/translations/messages.pot . --project=$(PROJECT) --version=$(VERSION) --copyright-holder=$(COPYRIGHT_HOLDER) --msgid-bugs-address=$(MSGID_BUGS_ADDRESS)

.PHONY: update-messages
update-messages: venv
	$(VENV_BIN)/pybabel update -i ./reforis_remote_wifi_settings/translations/messages.pot -d ./reforis_remote_wifi_settings/translations --update-header-comment
